class ProjectAssignmentsController < ApplicationController
  ...
  def index
    ...
    if params[:engineer_id]
       @project_assignments = ProjectAssignment.paginate :per_page => 5, :page => params[:page], :conditions => ['engineer_id = ?',params[:engineer_id]], :order => 'engineer_id'       
    elsif params[:project_id]
       @project_assignments = ProjectAssignment.paginate :per_page => 5, :page => params[:page], :conditions => ['project_id = ?',params[:project_id]], :order => 'project_id'
    else
       @project_assignments = ProjectAssignment.paginate :per_page => 5, :page => params[:page], :order => 'project_id'
    end
